...
class BanditTestSetTests(testtools.TestCase):
    ...
    def test_profile_has_builtin_banlist(self):
        ts = test_set.BanditTestSet(self.config)
    ...
    def test_profile_exclude_builtin_banlist(self):
    ...
    def test_profile_exclude_builtin_banlist_specific(self):
    ...
    def test_profile_filter_banlist_none(self):
        ts = test_set.BanditTestSet(self.config)
        banlist = ts.get_tests('Import')[0]

        self.assertEqual(2, len(banlist._config['Import']))
        self.assertEqual(2, len(banlist._config['ImportFrom']))
        self.assertEqual(2, len(banlist._config['Call']))

    def test_profile_filter_banlist_one(self):
        profile = {'exclude': ['B401']}
        ts = test_set.BanditTestSet(self.config, profile)
        banlist = ts.get_tests('Import')[0]

        self.assertEqual(1, len(banlist._config['Import']))
        self.assertEqual(1, len(banlist._config['ImportFrom']))
        self.assertEqual(1, len(banlist._config['Call']))

    def test_profile_filter_banlist_include(self):
    ...
    def test_profile_filter_banlist_all(self):
        profile = {'exclude': ['B401', 'B302']}
        ts = test_set.BanditTestSet(self.config, profile)

        # if there is no banlist data for a node type then we wont add a
        # banlist test to it, as this would be pointless.
        self.assertEqual(0, len(ts.get_tests('Import')))
        self.assertEqual(0, len(ts.get_tests('ImportFrom')))
        self.assertEqual(0, len(ts.get_tests('Call')))

    def test_profile_banlist_compat(self):
        data = [utils.build_conf_dict(
                'marshal', 'B302', ['marshal.load', 'marshal.loads'],
                ('Deserialization with the marshal module is possibly '
                 'dangerous.'))]

        profile = {'include': ['B001'], 'banlist': {'Call': data}}

        ts = test_set.BanditTestSet(self.config, profile)
        banlist = ts.get_tests('Call')[0]

        self.assertNotIn('Import', banlist._config)
        self.assertNotIn('ImportFrom', banlist._config)
        self.assertEqual(1, len(banlist._config['Call']))
